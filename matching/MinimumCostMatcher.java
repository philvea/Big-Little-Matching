...
/**
 * This minimum cost matcher solves the assignment problem in polynomial time using the hungarian method
 * Code adapted from:
 * 		https://www.sanfoundry.com/java-program-implement-hungarian-algorithm-bipartite-matching/
 * 
 * The Hungarian method used was discovered by H.W. Kuhn in a paper that can be found at: 
 * 		https://web.eecs.umich.edu/~pettie/matching/Kuhn-hungarian-assignment.pdf
 */
public class MinimumCostMatcher implements Matcher {
	...
	private LinkedHashMap <String, Little> littles;
	private LinkedHashMap <String, Big> bigs;
	
	public MinimumCostMatcher(double[][] costMatrix, LinkedHashMap <String, Little> littles, LinkedHashMap <String, Big> bigs) {
		this(costMatrix);
		this.littles = littles;
		this.bigs = bigs;
	}

	public MinimumCostMatcher(double[][] costMatrix)	{
	...
	public Collection<Pairing> makePairings() {
    	...
    	for (int i=0; i<littleOrdering.length; i++) {
    		...
    		Big big = bigs.get(bigOrdering[results[i]]);
    		
    		if (little.rankings.get(big) == null || big.rankings.get(little) == null) {
    			pairings.add(new Pairing(little, Big.unmatched()));
    		} else {
    			pairings.add(new Pairing(littles.get(littleOrdering[i]), bigs.get(bigOrdering[results[i]])));
    		}
    	}
		return pairings;
	}
